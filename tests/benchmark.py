"""
tests/benchmark.py — Enkidu evaluation + regression benchmark runner.
...
def _run_prompt(prompt_def: dict, latency_only: bool = False) -> dict:
    ...
    try:
        ...
        result["error"] = f"Could not import enkidu_agent: {e}"
...
def run_benchmark(
    ...
    if not as_json:
        print(f"\n  Enkidu Benchmark  run={run_id}  {timestamp}")
        print("  " + "─" * 60)
...
def show_trendline(n: int = 10):
    ...
    if not files:
        print("No scorecards yet.")
        return
    print(f"\n  Enkidu Score Trendline (last {len(files)} runs)")
    print("  " + "─" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Enkidu benchmark runner")
