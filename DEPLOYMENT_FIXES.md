...
1. **Auto-ref-text generation disabled by default** (`phase6-ui/server/voice.py`)
   - Changed: `ENKIDU_AUTO_REF_TEXT=1` → `ENKIDU_AUTO_REF_TEXT=0`
   - Reason: File iteration on startup could fail on Railway if voices/ directory has issues
   - To enable: Set `ENKIDU_AUTO_REF_TEXT=1` in Railway .env only after confirming server is stable
...
1. **Check Railway Logs:**
   - Railway dashboard → pachter-enkidu → Deployments → View Logs
...
```bash
# .env on Railway:
ENKIDU_AUTO_REF_TEXT=0              # Keep disabled until you confirm stability
ENKIDU_KOKORO_WARMUP=1              # Safe — just cuDNN tuning
ENKIDU_USE_PARAKEET=0               # Off by default — only enable if nemo installed
```
